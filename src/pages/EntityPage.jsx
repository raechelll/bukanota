...
export default function EntityPage({ route, openModal, notify, language = 'id' }) {
  ...
  const act = (row, action) => { if (action === 'detail') return openModal({ type: 'form', title: `Edit ${config.name}`, subtitle: 'Perbarui data, batalkan, atau hapus dari satu panel.', size: 'wide', fields: config.columns.map((column, i) => ({ name: `field-${i}`, label: column, value: row[i], type: column === 'Status' ? 'select' : 'text', options: column === 'Status' ? ['Aktif', 'Nonaktif', 'Pending'] : undefined, required: i < 2 })), detailActions: [...(config.resetAction ? [{ key: 'reset', label: 'Reset password', tone: 'warning' }] : [{ key: 'cancel', label: 'Batalkan data', tone: 'warning' }]), { key: 'delete', label: 'Hapus', tone: 'danger' }], onDetailAction: (nextAction) => act(row, nextAction), success: `${config.name} berhasil diperbarui` }); if (action === 'edit') add(); if (action === 'reset') openModal({ type: 'confirm', title: 'Reset password', message: `Reset password ${row[1]}?`, description: 'Password baru akan dikirim ke email user.', success: 'Password berhasil direset' }); if (action === 'cancel') openModal({ type: 'confirm', title: `Batalkan ${config.name}`, message: `Batalkan ${row[0]}?`, description: 'Status data akan diubah menjadi dibatalkan.', success: `${config.name} dibatalkan` }); if (action === 'delete') openModal({ type: 'confirm', title: `Hapus ${config.name}`, message: `Apakah Anda yakin ingin menghapus ${row[0]}?`, success: `${config.name} berhasil dihapus` }) }
