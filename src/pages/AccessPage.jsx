...
export default function AccessPage({ openModal, notify }) {
  ...
  const addRole = () => openModal({type:'form',title:'Tambah role',kicker:'ROLE & PERMISSION',fields:[{name:'name',label:'Nama role',placeholder:'Supervisor',required:true},{name:'copy',label:'Salin permission dari',type:'select',options:['Tidak ada',...roles.map(([r])=>r)]}],success:'Role berhasil ditambahkan',onConfirm:(data)=>{const name=(data.name||'').trim();if(!name||roles.some(([r])=>r.toLowerCase()===name.toLowerCase()))return;setRoles(rs=>[...rs,[name,0,'Role kustom tanpa deskripsi.']]);const copyFrom=data.copy&&data.copy!=='Tidak ada'?data.copy:null;setMatrix(m=>({...m,[name]:copyFrom&&m[copyFrom]?JSON.parse(JSON.stringify(m[copyFrom])):Object.fromEntries(modules.map(mod=>[mod,perms.map(()=>false)]))}))}})
  return <div className="module-page"><div className="access-grid"><aside><h3>Daftar peran <button className="access-add" onClick={addRole}><Plus /> Tambah</button></h3>{roles.map(([r,u])=><button className={role===r?'active':''} onClick={()=>setRole(r)} key={r}><span><UserCog /></span><div><strong>{r}</strong><small>{u} pengguna</small></div><ChevronRight /></button>)}</aside><section><div className="permission-head"><div><h3>{current[0]}</h3><p>{current[2]}</p></div><span><ShieldCheck /> {activeCount} izin aktif</span></div><div className="permission-matrix"><div className="permission-row head"><span>Module</span>{perms.map(p=><span key={p}>{p[0].toUpperCase()+p.slice(1)}</span>)}</div>{modules.map((module,i)=><div className="permission-row" key={module}><b>{module}</b>{perms.map((p,j)=><label key={p}><input type="checkbox" checked={matrix[role][module][j]} onChange={()=>toggle(i,j)} /><i></i></label>)}</div>)}</div><div className="permission-actions"><button onClick={()=>setAll(true)}>Select All</button><button onClick={reset}>Reset</button><button className="save-button" onClick={()=>notify(`Permission ${role} berhasil disimpan (${activeCount} izin aktif)`,'success')}>Simpan permission</button></div></section></div></div>
}
