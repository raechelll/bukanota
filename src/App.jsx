...
function InventoryPage({ openModal }) {
  ...
  return <div className="module-page"><div className="module-title"><div><span>INVENTORI</span><h2>Manajemen stok</h2><p>Pantau produk, supplier, dan pergerakan stok seluruh outlet.</p></div><button onClick={addProduct}><Plus /> Tambah {tab}</button></div><div className="inventory-kpis"><div><Package /><span><small>Total produk</small><strong>284</strong></span></div><div><Sparkles /><span><small>Stok menipis</small><strong>8 produk</strong></span></div><div><RefreshCcw /><span><small>Opname bulan ini</small><strong>3 aktivitas</strong></span></div></div><div className="module-tabs inventory-tabs">{tabs.map(t=><button className={tab===t?'active':''} onClick={()=>setTab(t)} key={t}>{t}</button>)}</div><div className="data-card"><div className="data-toolbar"><label><Search /><input placeholder={`Cari data ${tab.toLowerCase()}`} /></label><button><ListFilter /> Semua kategori <ChevronDown /></button><button><RefreshCcw /> Refresh</button><button><Download /> Export</button></div>{tab==='Produk'?<div className="data-table inventory-table"><div className="data-row data-head"><span>Produk</span><span>Kategori</span><span>Stok</span><span>Status</span><span>Aksi</span></div>{inventoryRows.map(([sku,name,cat,stock,status])=><div className="data-row" key={sku}><span><i className="table-thumb"></i><b>{name}<small>{sku}</small></b></span><span>{cat}</span><span><b>{stock}</b></span><span><em className={status==='Menipis'?'warning':''}>{status}</em></span><span><RowActions onAction={(a)=>action(name,a)} /></span></div>)}</div>:<div className="data-table inventory-subtable"><div className="data-row data-head"><span>Reference</span><span>Data</span><span>Qty / Status</span><span>Tanggal</span><span>Aksi</span></div>{[['REF-202608-001',`${tab} Kopi Susu`,'24 pcs','20 Agu 2026'],['REF-202608-002',`${tab} Matcha`,'12 pcs','19 Agu 2026'],['REF-202608-003',`${tab} Kemasan`,'Aktif','18 Agu 2026']].map(r=><div className="data-row" key={r[0]}>{r.map(v=><span key={v}>{v}</span>)}<span><RowActions onAction={(a)=>action(r[1],a)} /></span></div>)}</div>}<div className="table-footer"><span>Menampilkan 1–5 dari 284 data</span><div><button disabled>←</button><b>1</b><button>2</button><button>3</button><button>→</button></div></div></div></div>
}
...
function LogsPage({ openModal, notify }) {
  ...
  return <div className="module-page"><div className="module-title"><div><span>KEAMANAN</span><h2>Log activity</h2><p>Catatan aktivitas pengguna dan perubahan data.</p></div><button onClick={()=>notify('Export log demo disiapkan','info')}><Download /> Export log</button></div><div className="log-kpis"><div><Activity /><span><small>Aktivitas hari ini</small><strong>1.284</strong></span></div><div><Users /><span><small>Pengguna aktif</small><strong>18</strong></span></div><div><ShieldCheck /><span><small>Aktivitas mencurigakan</small><strong>0</strong></span></div></div><div className="data-card"><div className="data-toolbar"><label><Search /><input placeholder="Cari aktivitas" /></label><button><CalendarClock /> Hari ini</button><button><ListFilter /> Semua aktivitas <ChevronDown /></button></div><div className="data-table log-table with-actions"><div className="data-row data-head"><span>Waktu</span><span>Pengguna</span><span>Aktivitas</span><span>Detail</span><span>IP Address</span><span>Aksi</span></div>{logs.map(r => <div className="data-row" key={r[0]}><span><Clock3 />{r[0]}</span><span><b>{r[1]}</b></span><span><em className={`log-${r[2].toLowerCase()}`}>{r[2]}</em></span><span>{r[3]}</span><span>{r[4]}</span><span><RowActions onAction={(a)=>a==='detail'?detail(r):notify('Log aktivitas bersifat read-only','warning')} /></span></div>)}</div></div></div>
}
...
function EntityPage({ route, openModal }) {
  ...
  return <div className="module-page"><div className="module-title"><div><span>MODUL BUKANOTA</span><h2>{config.name}</h2><p>Kelola data {config.name.toLowerCase()} dengan pencarian, filter, dan action modal.</p></div><button onClick={add}><Plus /> Tambah data</button></div><div className="entity-stats">{config.stats.map(([n,v],i)=><div key={n}><span className={`metric-icon m-${i}`}><Icon /></span><small>{n}</small><strong>{v}</strong></div>)}</div><div className="data-card"><div className="data-toolbar"><label><Search /><input placeholder={`Cari ${config.name.toLowerCase()}`} /></label><button><CalendarClock /> Tanggal <ChevronDown /></button><button><Building2 /> Outlet <ChevronDown /></button><button><ListFilter /> Filter <ChevronDown /></button><button><Download /> Export <ChevronDown /></button></div><div className="data-table entity-table" style={{'--columns':config.columns.length}}><div className="data-row data-head">{config.columns.map(c=><span key={c}>{c}</span>)}<span>Aksi</span></div>{config.rows.map(row=><div className="data-row" key={row[0]}>{row.map((value,i)=><span key={`${row[0]}-${i}`}>{i===0?<b>{value}</b>:i===row.length-1?<em className={['Pending','Nonaktif','Failed'].includes(value)?'warning':''}>{value}</em>:value}</span>)}<span><RowActions extended onAction={(a)=>act(row,a)} /></span></div>)}</div><div className="table-footer"><span>Menampilkan 1–3 dari 128 data</span><div><button disabled>←</button><b>1</b><button>2</button><button>→</button></div></div></div></div>
}
